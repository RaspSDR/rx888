...
uint32_t EventAvailableQueue[16]
    __attribute__((aligned(32))); // Queue for up to 16 uint32_t pointers
...
uint16_t FWconfig =
    (FIRMWARE_VER_MAJOR << 8) | FIRMWARE_VER_MINOR; // Firmware rc1 ver 1.02
...
void MsgParsing(uint32_t qevent) {
  ...
  case 1:
    DebugPrint(4, "\r\nVendor request = %x  %x  %x\r\n",
               (uint8_t)(qevent >> 16), (uint8_t)(qevent >> 8),
               (uint8_t)qevent);
    break;
  case 2:
    DebugPrint(4, "\r\nfree \r\n", (uint8_t)qevent);
...
void ApplicationThread(uint32_t input) {
  // input is passed to this routine from CyU3PThreadCreate, useful if the same
  // code is used for multiple threads
#ifndef _DEBUG_USB_
  ...
  if (Status != CY_U3P_SUCCESS)
    ...
    if (Status != CY_U3P_SUCCESS) {
      ...
      CyU3PGpioSimpleGetValue(GPIO52, &measure); // measure if external pull
                                                 // down
      if (measure) {
        HWconfig = HF103;
      ...
      uint8_t identity;
      if (I2cTransfer(0, R820T_I2C_ADDR, 1, &identity, true) ==
          CY_U3P_SUCCESS) {
      ...
      } else if (I2cTransfer(0, R828D_I2C_ADDR, 1, &identity, true) ==
                 CY_U3P_SUCCESS) {
      ...
      } else if (I2cTransfer(0, RD5812_I2C_ADDR, 1, &identity, true) ==
                 CY_U3P_SUCCESS) {
        HWconfig = RX888r3;
  ...
  if (Status == CY_U3P_SUCCESS) {
    ...
    while (1) {
      // for (Count = 0; Count<10; Count++)
      {
        ...
        CyU3PThreadSleep(100);
        nline = 0;
        while (CyU3PQueueReceive(&EventAvailable, &Qevent, CYU3P_NO_WAIT) ==
               0) {
          if (nline++ == 0)
            DebugPrint(4, "\r\n"); // first line
          MsgParsing(Qevent);
...
#ifndef _DEBUG_USB_ // #include "../Interface.h"  -> second count in serial
                    // debug
      if (glDMACount > 7812) {
        glDMACount -= 7812;
        DebugPrint(4, "%d, \n", Seconds++);
  ...
  DebugPrint(4, "\r\nApplication failed to initialize. Error code: %d.\r\n",
             Status);
  // Returning here will stop the Application Thread running - it failed anyway
  // so this is OK
}
...
void CyFxApplicationDefine(void) {
  ...
  Status = CyU3PQueueCreate(&EventAvailable, 1, &EventAvailableQueue,
                            sizeof(EventAvailableQueue));
  CheckStatus("Create EventAvailableQueue", Status);

  StackPtr[0] = CyU3PMemAlloc(FIFO_THREAD_STACK);
  Status = CyU3PThreadCreate(
      &ThreadHandle[0],     /* Slave FIFO app thread structure */
