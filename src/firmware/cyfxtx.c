...
/* File: cyfxtx.c
 *
 * This file provides the application specific exception handlers and memory
 * allocation routines. A sample implementation is provided with the FX3 SDK and
 * can be updated where required by the application.
 *
 * Note: Please do not make changes to the signatures of the functions; as the
 * drivers in the SDK depends on these functions. The implementation can be
 * changed where required.
 *
 * This file has been updated with some new features related to memory leak and
 * corruption detection. These changes are only enabled when compiling with SDK
...
#if ((CYFX_VERSION_MINOR > 3) ||                                               \
     ((CYFX_VERSION_MINOR == 3) && (CYFX_VERSION_PATCH >= 3)))
...
/*
   ...
   Note: The 2-stage boot area is optional (only required if the application
   makes use of a persistent in-memory boot-loader). If this is not being used,
   the 32 KB reserved for this segment can be merged into the buffer area by
   changing CY_U3P_SYS_MEM_TOP to 0x40040000.
 */
...
/*
   ...
   makes use of a persistent in-memory boot-loader). If this is not being used,
   the 32 KB reserved for this segment can be merged into the buffer area by
   changing CY_U3P_SYS_MEM_TOP to 0x40080000.
 */
...
static CyBool_t glMemPoolInit =
    CyFalse; /* Whether the memory allocator has been initialized. */
static CyU3PBytePool
    glMemBytePool; /* ThreadX Byte pool used in the CyU3PMem* functions. */
static CyU3PDmaBufMgr_t glBufferManager = {
    {0}, 0, 0,
    0,   0, 0}; /* Buffer manager used in the buffer alloc functions. */

#ifdef CYFXTX_ERRORDETECTION

/*
   Debug variables used for doing memory leak and corruption checks around
   buffers allocated through the CyU3PMemAlloc function.
 */
...
static CyU3PMemCorruptCallback glMemBadCb =
    0; /* Callback for notification of corrupted memory. */

/*
   Debug variables used for doing memory leak and corruption checks around
   buffers allocated through the CyU3PDmaBufferAlloc function.
 */
static CyBool_t glBufMgrEnableChecks =
    CyFalse;                       /* Whether checks are enabled. */
...
static CyU3PMemCorruptCallback glBufBadCb =
    0; /* Callback for notification of corrupted memory. */
...
/* These functions are standard ARM9 exception handlers for various memory
 * access errors. Default implementations which map to a "while (1) {}" are
 * provided here, as it is not possible to define meaningful error handling in
 * an application agnostic manner.
 *
 * These can be replaced with functions that update LEDs/GPIOs, use DebugPrint
 * or even reset FX3 as required by the application.
 */

/* Function    : CyU3PUndefinedHandler
 * Description : Handler for an undefined instruction exceptions. This is only
 *               expected to be triggered when there is a build settings
 * conflict or memory corruption happening on FX3. Parameters  : None
 */
 ...
 * Description : Handler for a data abort error. As virtual memory is not used
 * by the SDK, this error can only be triggered when there is memory corruption
 * or there is an access to an uninitialized memory pointer. Parameters  : None
 */
 ...
 * Description : This is a ThreadX RTOS defined function that is called once the
 * RTOS scheduler is initialized. This function has to be provided in this file
 * and is expected to call CyU3PApplicationDefine() so that the SDK internal
 * drivers can be initialized.
 *
 *               The CyFxApplicationDefine function will be called once the SDK
 * internal drivers have been started up. This function can be used to perform
 * any initialization that needs to happen before CyFxApplicationDefine() is
 *               called.
 *
 * Parameters  : Pointer to the first un-initialized memory. This is not
 * expected to be used.
 */
 ...
 * Description  : Enable memory leak and corruption checks in the driver heap
 * allocator. Enabling the checks will cause the memory required for each
 * allocated block to increase by 24 bytes; and the allocation operation to take
 *                additional time.
 * Parameters   :
 *                enable : Whether to enable memory leak and corruption checks.
 *                cb     : Callback function to be called when the allocator
 * detects memory corruption. Return Value : CY_U3P_SUCCESS if the
 * enable/disable is performed correctly. CY_U3P_ERROR_ALREADY_STARTED if the
 * CyU3PMemInit function has already been called.
 */
CyU3PReturnStatus_t CyU3PMemEnableChecks(CyBool_t enable,
                                         CyU3PMemCorruptCallback cb) {
  CyU3PReturnStatus_t stat = CY_U3P_ERROR_ALREADY_STARTED;

  /* We can enable/disable checks only before the CyU3PMemInit function has been
   * called. */
  if (!glMemPoolInit) {
 ...
 * Description : This function initializes the custom heap for OS specific
 * dynamic memory allocation. The function should not be explicitly invoked, and
 * is called from the API library. The minimum required size for the heap is 20
 * KB. The default implementation makes use of the Byte Pool services provided
 *               by ThreadX.
 * Parameters  : None
 */
void CyU3PMemInit(void) {
  ...
  if (!glMemPoolInit) {
    ...
    CyU3PBytePoolCreate(&glMemBytePool, (void *)CY_U3P_MEM_HEAP_BASE,
                        CY_U3P_MEM_HEAP_SIZE);
  }
}

/* Function     : CyU3PMemAlloc
 * Description  : This function allocates memory required for various OS objects
 * in the firmware application. This function is used by the SDK internal
 * drivers in addition to the application code itself. The default
 * implementation makes use of the ThreadX byte pool services. If memory leak
 * and corruption checking is enabled, the implementation adds a 20 byte header
 * and a 4 byte footer around the memory block. Parameters   : size : Size of
 * memory required in bytes. Return Value : Pointer to the allocated memory
 * block.
 */
 ...
 * Description : This function frees memory previously allocated using
 * CyU3PMemAlloc. Parameters  : size : Pointer to memory block to be freed.
 */
void CyU3PMemFree(void *mem_p) {
  ...
  if (glMemEnableChecks) {
    ...
    endsig_p = (uint32_t *)((uint8_t *)block_p + block_p->alloc_size -
                            sizeof(uint32_t));

    if ((block_p->start_sig != CY_U3P_MEM_START_SIG) ||
        (*endsig_p != CY_U3P_MEM_END_SIG)) {
 ...
 * Parameters   :
 *                allocCnt_p : Parameter to be filled with number of
 * CyU3PMemAlloc calls. freeCnt_p  : Parameter to be filled with number of
 * CyU3PMemFree calls. Return Value : None
 */
 ...
 *                check for memory leaks leading to allocation failure at
 * runtime. Parameters   : None Return Value : Pointer to the currently in-use
 * memory blocks. All of the blocks can be identified by traversing the list
 * using the prev_blk pointer in the MemBlockInfo structure. Note         : The
 * active list will contain memory blocks allocated by the FX3 SDK internal
 * drivers. These can be identified by keeping track of head of the list when
 * the CyFxApplicationDefine function is called.
 */
MemBlockInfo *CyU3PMemGetActiveList(void) { return glMemInUseList; }
...
CyU3PReturnStatus_t CyU3PMemCorruptionCheck(void) {
  ...
  /* Run through all in-use memory blocks and send a callback for any blocks
     that do not match the start and end signatures.
   */
  block_p = glMemInUseList;
  while (block_p != 0) {
    if (((uint32_t)block_p < CY_U3P_MEM_HEAP_BASE) ||
        ((uint32_t)block_p >= CY_U3P_BUFFER_HEAP_BASE))
      return CY_U3P_ERROR_FAILURE;

    mem_p = (uint32_t *)((uint8_t *)block_p + block_p->alloc_size -
                         sizeof(uint32_t));
    if ((block_p->start_sig != CY_U3P_MEM_START_SIG) ||
        (*mem_p != CY_U3P_MEM_END_SIG)) {
      if (glMemBadCb != 0)
        glMemBadCb((void *)((uint8_t *)block_p + sizeof(MemBlockInfo)));

      /* Once we find any corruption, we cannot rely on the list pointers any
       * more. */
      return CY_U3P_ERROR_FAILURE;
    }
 ...
 *                No checks are performed on the parameters because even a
 * NULL-pointer is valid on the FX3 device. Parameters   : ptr   : Pointer to
 * memory block to be initialized. data  : Value that should be set at each
 * byte. count : Size of memory block. Return Value : None
 */
 ...
 * Description  : memcpy equivalent function to copy one memory block to
 * another. This function assumes that the memory block may not be DWORD
 *                aligned; and performs a byte-by-byte copy.
 *                No checks are performed on the parameters because even a
 * NULL-pointer is valid on the FX3 device. Parameters   : dest  : Pointer to
 * destination memory block. src   : Pointer to source memory block. count :
 * Size of memory block. Return Value : None
 */
void CyU3PMemCopy(uint8_t *dest, uint8_t *src, uint32_t count) {
  if (dest > src) {
    /* Destination buffer is above source buffer. Copy from end of the buffer
     * back to the start. */
    dest += count;
    src += count;
    ...
    /* Destination buffer is below source buffer. Copy from start to end of the
     * buffer. */
 ...
 *                Difference between first non-identical byte in case of
 * deviation.
 */
 ...
 * Description  : Enable memory leak and corruption checks in the buffer heap
 * allocator. Enabling the checks will cause the memory required for each
 * allocated block to increase by 24 bytes; and the allocation operation to take
 *                additional time.
 * Parameters   :
 ...
 * detects memory corruption. Return Value : CY_U3P_SUCCESS if the
 * enable/disable is performed correctly. CY_U3P_ERROR_ALREADY_STARTED if the
 * CyU3PDmaBufferInit function has already been called.
 */
CyU3PReturnStatus_t CyU3PBufEnableChecks(CyBool_t enable,
                                         CyU3PMemCorruptCallback cb) {
  CyU3PReturnStatus_t stat = CY_U3P_ERROR_ALREADY_STARTED;
 ...
 * Description : This function initializes the custom heap used for DMA buffer
 * allocation. These functions use a home-grown allocator in order to ensure
 * that all DMA buffers allocated are cache line aligned (multiple of 32 bytes).
 *               The function should not be explicitly invoked, and is called
 * from the API library. Parameters  : None
 */
void CyU3PDmaBufferInit(void) {
  ...
  glBufferManager.usedStatus =
      (uint32_t *)CyU3PMemAlloc(size * sizeof(uint32_t));
  if (glBufferManager.usedStatus == 0) {
  ...
  CyU3PMemSet((uint8_t *)glBufferManager.usedStatus, 0,
 ...
 * Description : This function frees up the custom heap used for DMA buffer
 * allocation. The function should not be explicitly invoked, and is called from
 * the API library. Parameters  : None
 */
 ...
 * Description  : This function allocates memory required for DMA buffers
 * required by the firmware application. This function is used by the SDK
 * internal drivers in addition to the application code itself. If memory leak
 * and corruption checking is enabled, the implementation adds a 20 byte header
 * and a 4 byte footer around each memory block. Parameters   : size : Size of
 * memory required in bytes. Return Value : Pointer to the allocated memory
 * block.
 */
void *CyU3PDmaBufferAlloc(uint16_t size) {
  ...
  if (glBufMgrEnableChecks) {
    /* Using a 32-bit variable here to allow for addition of header on top of a
     * maximum sized allocation. */
    blk_size = ROUND_UP(blk_size, 4);
    blk_size += sizeof(MemBlockInfo) + sizeof(uint32_t);
  }
#endif

  /* Find the number of cache lines required. The minimum size that can be
   * handled is 2 cache lines. */
  size = (blk_size <= FX3_CACHE_LINE_SZ)
             ? 2
             : ((blk_size + FX3_CACHE_LINE_SZ - 1) / FX3_CACHE_LINE_SZ);
 ...
 * Description  : This function frees memory previously allocated using
 * CyU3PDmaBufferAlloc. Parameters   : buffer : Pointer to memory block to be
 * freed. Return Value : 0 if free is successful, non-zero error code in case of
 * mutex failure.
 */
int CyU3PDmaBufferFree(void *buffer) {
  ...
  if (glBufMgrEnableChecks) {
    ...
    sig_p = (uint32_t *)((uint8_t *)block_p + block_p->alloc_size -
                         sizeof(uint32_t));
    if ((block_p->start_sig != CY_U3P_MEM_START_SIG) ||
        (*sig_p != CY_U3P_MEM_END_SIG)) {
      /* Notify the user that memory has been corrupted. */
      if (glBufBadCb != 0)
        glBufBadCb(buffer);
    }
  ...
  if ((start > glBufferManager.startAddr) &&
    ...
    /* Start the next buffer search at the top of the heap. This can help reduce
       fragmentation in cases where most of the heap is allocated and then freed
       as a whole. */
    glBufferManager.searchPos = 0;
    retVal = 0;
  }
 ...
 * Description : This function de-initializes both driver and buffer heap
 * allocators. This is called from the SDK library and is not expected to be
 * called from user code. Parameters  : None
 */
 ...
 * Parameters   :
 *                allocCnt_p : Parameter to be filled with number of
 * CyU3PDmaBufferAlloc calls. freeCnt_p  : Parameter to be filled with number of
 * CyU3PDmaBufferFree calls. Return Value : None
 */
 ...
 * memory blocks. All of the blocks can be identified by traversing the list
 * using the prev_blk pointer in the MemBlockInfo structure. Note         : The
 * active list may contain blocks that are allocated by the CyU3PDebugInit and
 * CyU3PUsbStart APIs (8 and 2 buffers respectively).
 */
MemBlockInfo *CyU3PBufGetActiveList(void) { return glBufInUseList; }
...
CyU3PReturnStatus_t CyU3PBufCorruptionCheck(void) {
  ...
  while (block_p != 0) {
    if (((uint32_t)block_p < CY_U3P_BUFFER_HEAP_BASE) ||
        ((uint32_t)block_p >= CY_U3P_SYS_MEM_TOP))
      return CY_U3P_ERROR_FAILURE;
