...
CyU3PDmaMultiChannel
    glMultiChHandleSlFifoPtoU; /* DMA Channel handle for P2U transfer. */
...
void StartApplication(void) {
  ...
  // Start GPIF clocks, they need to be running before we attach a DMA channel
  // to GPIF
  pibClock.clkDiv = 2;
  pibClock.clkSrc = CY_U3P_SYS_CLK;
  pibClock.isHalfDiv = CyFalse;
  pibClock.isDllEnable =
      CyFalse; // Disable Dll since this application is synchronous
  ...
  Status =
      CyU3PDmaMultiChannelSetXfer(&glMultiChHandleSlFifoPtoU, FIFO_DMA_RX_SIZE,
                                  0); /* DMA transfer size is set to infinite */
