...
#define CYFX_SDRAPP_MAX_EP0LEN                                                 \
  64 /* Max. data length supported for EP0 requests. */

extern CyU3PDmaMultiChannel glMultiChHandleSlFifoPtoU;
// Global data owned by this module
CyU3PDmaChannel glGPIF2USB_Handle;
uint8_t *glEp0Buffer =
    0; /* Buffer used to handle vendor specific control requests. */
...
/* Trace function */
void TraceSerial(uint8_t bRequest, uint8_t *pdata, uint16_t wValue,
                 uint16_t wIndex) {
...
CyBool_t CyFxSlFifoApplnUSBSetupCB(uint32_t setupdat0, uint32_t setupdat1) {
  ...
  if (bType == CY_U3P_USB_STANDARD_RQT) {
     ...
     * configured state and failed otherwise. */
     ...
     * regardless of the enumeration model used. When a clear feature is
     * received, the previous transfer has to be flushed and cleaned up. This is
     * done at the protocol level. So flush the EP memory and reset the DMA
     * channel associated with it. If there are more than one EP associated with
     * the channel reset both the EPs. The endpoint stall and toggle / sequence
     * number is also expected to be reset. Return CyFalse to make the library
     * clear the stall and reset the endpoint toggle. Or invoke the
     * CyU3PUsbStall (ep, CyFalse, CyTrue) and return CyTrue. Here we are
     * clearing the stall. */
    if ((bTarget == CY_U3P_USB_TARGET_ENDPT) &&
      ...
      if (glIsApplnActive) {
        if (wIndex == CY_FX_EP_CONSUMER) {
          ...
          CyU3PDmaMultiChannelSetXfer(&glMultiChHandleSlFifoPtoU,
                                      FIFO_DMA_RX_SIZE, 0);
        }
    ...
    case I2CWFX3:
      ...
      if (apiRetStatus == CY_U3P_SUCCESS) {
        apiRetStatus =
            I2cTransfer(wIndex, wValue, wLength, glEp0Buffer, CyFalse);
        if (apiRetStatus == CY_U3P_SUCCESS)
          isHandled = CyTrue;
        else {
    ...
    case STARTFX3:
      ...
      CyU3PDmaMultiChannelReset(
          &glMultiChHandleSlFifoPtoU); // Reset existing channel
      apiRetStatus = CyU3PDmaMultiChannelSetXfer(&glMultiChHandleSlFifoPtoU,
                                                 FIFO_DMA_RX_SIZE, 0); // start
      if (apiRetStatus == CY_U3P_SUCCESS) {
        isHandled = CyTrue;
      }
    ...
    case READINFODEBUG: {
      if (wValue > 0) {
        ...
        if (InputChar == 0x0d) {
          ...
          ConsoleInBuffer[ConsoleInIndex] =
              InputChar | 0x20; // Save character as lower case (for compares)
...
/* Callback function to handle LPM requests from the USB 3.0 host. This function
   ...
   happens. If we return CyTrue from this function, the FX3 device is retained
   in the low power state. If we return CyFalse, the FX3 device immediately
   tries to trigger an exit back to U0. This application does not have any state
   in which we should not allow U1/U2 transitions; and therefore the function
   always return CyTrue.
 */
