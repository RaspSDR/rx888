...
extern CyU3PThread
    ThreadHandle[APP_THREADS];      // Handles to my Application Threads
...
const char *FX3CommandName[] = { // start 0xAA
    "STARTFX3",  "STOPFX3",      "TESTFX3",   "GPIOFX3",  "I2CWFX3",
    "I2CRFX3",   "0xB0",         "RESETFX3",  "STARTADC", "0xB3",
    "TUNERINIT", "TUNERTUNE",    "SETARGFX3", "0xB7",     "TUNERSTDBY",
    "0xB9",      "READINFODEBUG"};

// For Debug and display the name of the FX3Command
const char *SETARGFX3List[] = {"0",
                               "R82XX_ATTENUATOR",
                               "R82XX_VGA",
                               "R82XX_SIDEBAND",
                               "R82XX_HARMONIC",
...
void ParseCommand(void) {
  ...
  if (!strcmp("", ConsoleInBuffer) || !strcmp("?", ConsoleInBuffer)) {
    ...
    tx_thread_info_get(ThisThread, &ThreadName, NULL, NULL, NULL, NULL, NULL,
                       &NextThread, NULL);
    // Now, using the Thread linked list, look for other threads until I find
    // myself again
    DebugPrint(4, "This : '%s'", ThreadName);
    while (NextThread != ThisThread) {
      tx_thread_info_get(NextThread, &ThreadName, NULL, NULL, NULL, NULL, NULL,
                         &NextThread, NULL);
      DebugPrint(4, "\r\nFound: '%s'", ThreadName);
    }
    DebugPrint(4, "\r\n\r\n");
    ...
    DebugPrint(4, "Stack free in %s is %d/%d\r\n\r\n", ThreadName,
               FIFO_THREAD_STACK - ((DataPtr - StackStartPtr) << 2),
               FIFO_THREAD_STACK);
    ...
    DebugPrint(4, "Input: '%s'\r\n\r\n", &ConsoleInBuffer[0]);
  ConsoleInIndex = 0;
}

uint8_t *CyU3PDebugIntToStr(uint8_t *convertedString, uint32_t num,
                            uint8_t base);

// from /FX3_SDK_1_3_1_SRC/sdk/firmware/src/system/cyu3debug.c
// MyDebugSNPrint

static CyU3PReturnStatus_t MyDebugSNPrint(uint8_t *debugMsg, uint16_t *length,
                                          char *message, va_list argp) {
  uint8_t *string_p;
  uint8_t *argStr = NULL;
...
void DebugPrint2USB(uint8_t priority, char *msg, ...) {
  if ((glIsApplnActive != CyTrue) || (flagdebug == false))
    return;
  va_list argp;
...
// Handle characters typed in by the developer
{
  ...
  if (Event == CY_U3P_UART_EVENT_RX_DONE) {
    ...
    // On CR, signal Main loop to process the command entered by the user.
    // Should NOT do this in a CallBack routine
    if (InputChar == 0x0d) {
...
CyU3PReturnStatus_t InitializeDebugConsole(void) {
  ...
  CheckStatus("CyU3PUartInit",
              Status); // This will not display since we're not connected yet
  ...
  Status = CyU3PDebugInit(CY_U3P_LPP_SOCKET_UART_CONS,
                          8); // Attach the Debug driver above the UART driver
