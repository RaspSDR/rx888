...
void IndicateError(uint16_t ErrorCode) {
  ...
  /*
      ...
      CyU3PGpioComplexConfig_t gpioConfig;
          // LED is on UART_CTS which is currently been assigned to the UART
     driver, claim it back CyU3PDeviceGpioOverride(UART_CTS, CyFalse);
     ...
     CY_U3P_GPIO_MODE_PWM; gpioConfig.timerMode = CY_U3P_GPIO_TIMER_HIGH_FREQ;
...
int main(void) {
  ...
  // The default clock runs at 384MHz, adjust it up to 403MHz so that GPIF can
  // be "100MHz"
  clkCfg.setSysClk400 =
      CyTrue; /* FX3 device's master clock is set to a frequency > 400 MHz */
  ...
  if (Status == CY_U3P_SUCCESS) {
    ...
    if (Status == CY_U3P_SUCCESS) {
      /* Configure the IO matrix for the device. On the FX3 DVK board, the COM
       * port is connected to the IO(53:56). This means that either DQ32 mode
       * should be selected or lppMode should be set to UART_ONLY. Here we are
       * choosing UART_ONLY configuration for 16 bit slave FIFO configuration
       * and setting isDQ32Bit for 32-bit slave FIFO configuration. */
      ioConfig.useUart = CyTrue;
      ioConfig.useI2C = CyTrue;
      ioConfig.useI2S = CyFalse;
      ioConfig.useSpi = CyFalse;
      ioConfig.isDQ32Bit =
          CyFalse; // CY_FX_SLFIFO_GPIF_16_32BIT_CONF_SELECT == 0 -> 16 bit
      ...
      //	if (CyU3PDeviceConfigureIOMatrix (&ioConfig) == CY_U3P_SUCCESS)
      //CyU3PKernelEntry ();
